<script lang="ts">
    ...
    function downloadPNG() {
        exportPNG(chartDiv, `distribution-${Date.now()}.png`);
    }
    function downloadSVG() {
        exportSVG(chartDiv, `distribution-${Date.now()}.svg`);
    }
</script>
...
<div class="flex flex-col h-full">
    ...
    <div bind:this={chartDiv} use:echartAction={{ option }} role="img" aria-label="Distribution chart of practice volume" class="flex-1 w-full h-full min-h-100 border border-[#E5E7EB] rounded"></div>
</div>
