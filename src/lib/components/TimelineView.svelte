<script lang="ts">
    ...
    const isGridMode = $derived(engine.timelineConfig.splitBy !== 'none' && engine.timelineConfig.useChartGrid);

    let chartDiv = $state<HTMLDivElement>();
    // Single-chart (non-grid) option — the first (and usually only) segment.
    const option: EChartsOption = $derived.by(() => timelineSegments[0]?.option ?? {});
...
<div class="flex flex-col h-full p-2 space-y-3 overflow-y-auto">
    ...
    {:else}
        <div bind:this={chartDiv} use:echartAction={{ option }} role="img" aria-label="Timeline chart of practice volume over time" class="flex-1 w-full min-h-100 border border-[#E5E7EB] rounded"></div>
    {/if}
</div>
