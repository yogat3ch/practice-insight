<script lang="ts">
	...
	async function loadSample(): Promise<void> {
		...
		try {
			...
			errorMessage = err instanceof Error ? err.message : 'Failed to load sample CSV.';
		} finally {
			isLoading = false;
		}
	}
...
<section aria-labelledby="csvCardTitle" class="border border-[#E5E5E5] bg-white rounded-md p-3">
	<h3 id="csvCardTitle" class="text-sm font-semibold text-[#1C1C1C] mb-2">CSV Data</h3>
	...
	>
		...
		/>
		{#if isLoading}
			<div class="flex items-center justify-center gap-2 text-sm text-[#6E6E6E]">
		...
		{:else}
			<svg class="w-6 h-6 mx-auto mb-1 text-[#6E6E6E]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 13M9 17h1m4 0h1m-6 4h6m-4-4v4"/>
			</svg>
			<p class="text-sm text-[#6E6E6E]">Drag &amp; drop your CSV here, or click to browse</p>
		{/if}
	</div>
	...
	{#if hasLoaded}
		<p class="mt-2 text-sm text-[#1C1C1C]">
			...
			{#if skippedCount > 0}
				<span class="text-[#6E6E6E]">({formatCount(skippedCount)} invalid, skipped)</span>
			{/if}
		</p>
	{:else}
	...
	{#if errorMessage}
		<p role="alert" class="mt-2 text-xs text-[#B45309] bg-amber-100 rounded px-2 py-1.5">
			{errorMessage}
		</p>
	{/if}
