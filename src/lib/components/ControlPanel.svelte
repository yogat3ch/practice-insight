<script lang="ts">
	...
	let dateFrom = $state<string>(
		engine.filters.dateFrom
			? format(engine.filters.dateFrom, 'yyyy-MM-dd')
			: '',
	);
...
<div class="p-4 space-y-4">
	...
	<Accordion
		...
		defaultOpen={true}
		count={selectedActivities.length + selectedPresets.length}
	>
		<div>
			<span
			...
			>
				{#if activities.length === 0}
					<p class="px-2 py-1.5 text-xs text-[#6E6E6E]">
				...
				{:else}
					{#each activities as act}
						...
						<button
							...
							class="block w-full text-left px-2 text-sm transition-colors focus:outline-none focus-visible:bg-[#EAA845]/20 {isSelected
								? 'bg-[#EAA845]/15 text-[#1C1C1C]'
								: 'text-[#1C1C1C] hover:bg-[#F9FAFB]'}"
						>
							<span class="inline-flex items-center gap-2">
								<span
									class="inline-block w-3 h-3 rounded border {isSelected
			...
			<div class="mt-1.5 flex items-center gap-3 text-xs">
				<button
					...
					class="font-semibold text-emerald-600 hover:text-emerald-500 hover:underline focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 rounded transition-colors"
				>
					Select All
				</button>
				<button
		...
		<button
			...
			class="mt-2 w-full px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-md font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2"
		>
			Apply Filters
		</button>
	</Accordion>
