...
function makeBreakdownItems(): CategoryBreakdownItem[] {
	return [
		{ name: 'Meditation', sessionCount: 3, totalValue: 90, averageValue: 30, percentage: 60 },
		{ name: 'Yoga', sessionCount: 2, totalValue: 60, averageValue: 30, percentage: 40 }
	];
}
...
describe('compileDayOfWeekOption — bar chart', () => {
	it('uses total duration values by default', () => {
		const opt = compileDayOfWeekOption(makeDayBins([10, 20, 30]), 'minutes', 'bar');
	...
	it('switches to session counts when metric is sessionCount', () => {
		const opt = compileDayOfWeekOption(makeDayBins([10, 20, 30]), 'minutes', 'bar', 'sessionCount');
		const series = opt.series as Array<{ data: number[] }>;
...
describe('compileDayOfWeekOption — heatmap', () => {
	it('emits a single-row heatmap with cool-to-warm visualMap', () => {
		const opt = compileDayOfWeekOption(makeDayBins([10, 20]), 'minutes', 'heatmap');
...
describe('compileDayOfWeekHeatmapMatrix (§5.3 temporal grouping)', () => {
	it('builds one row per temporal period', () => {
		...
		const opt = compileDayOfWeekHeatmapMatrix(periodBins, 'minutes', 'totalDuration', 'month');
	...
	it('applies the metric to each cell', () => {
		const periodBins = [{ period: 'Jul 2026', bins: makeDayBins([10, 20, 0, 0, 0, 0, 0]) }];
		const opt = compileDayOfWeekHeatmapMatrix(periodBins, 'minutes', 'sessionCount', 'month');
		const series = opt.series as Array<{ data: [number, number, number][] }>;
		// 10 minutes / 2 sessions = 5 average for Monday.
		const avgOpt = compileDayOfWeekHeatmapMatrix(periodBins, 'minutes', 'averageDuration', 'month');
		const avgSeries = avgOpt.series as Array<{ data: [number, number, number][] }>;
...
describe('compileTimeOfDayOption', () => {
	it('emits an hourly histogram with 24 bins', () => {
		const opt = compileTimeOfDayOption(makeHourBins([5, 10]), 'hours', 'histogram');
...
describe('compileCategoryBreakdownOption', () => {
	it('emits a donut when style is donut', () => {
		const opt = compileCategoryBreakdownOption(makeBreakdownItems(), 'minutes', 'donut');
	...
	it('emits stacked bar segments per category when style is stackedBar', () => {
		const opt = compileCategoryBreakdownOption(makeBreakdownItems(), 'minutes', 'stackedBar');
	...
	it('emits one stacked bar per temporal period with per-period segments', () => {
		const periodItems = [
			...
			},
			{
				period: 'Aug 2026',
				items: [
					{ name: 'Meditation', sessionCount: 1, totalValue: 30, averageValue: 30, percentage: 100 }
				]
			}
		];
		const opt = compileCategoryStackedBar(periodItems, 'minutes', 'totalDuration', 'month');
...
describe('per-period calculator integration', () => {
	...
	it('groups day-of-week bins by month into periods', () => {
		const periods = computeDayOfWeekPeriodDistribution(sessions, 'minutes', 0, 'month');
	...
	it('groups category breakdown by quarter into periods', () => {
		const periods = computeCategoryPeriodBreakdown(sessions, 'minutes', 'activity', 0, 'quarter');
		expect(periods.length).toBe(1);
	...
	it('honors the threshold by excluding short sessions', () => {
		...
		const periods = computeCategoryPeriodBreakdown(sessions, 'minutes', 'activity', 20, 'month');
		expect(periods.length).toBe(1);
		const yoga = periods[0].items.find((i) => i.name === 'Yoga');
